...
import { createRouteScopedSnapshot, normalizeCollection, normalizeConclusion, normalizeV3Message, normalizeV3Session, protectUnauthenticatedLiveSnapshot, sanitizePublicValue, summarizePerformanceTelemetry, templateEndpointPath } from './data-utils.js';
...
export async function getHonchoSnapshot() {
  ...
  if (env.USE_DEMO_DATA || !env.ALLOW_LIVE_PUBLIC_DATA) {
    ...
    const kanban = env.USE_DEMO_DATA
      ? undefined
      : getKanbanRuntimeSnapshot({ env: process.env, generatedAt });
  ...
  return protectUnauthenticatedLiveSnapshot({
    source: failures.length ? 'live-partial' : 'live',
    readOnly: true,
    env: publicEnv,
    status: { ok: failures.length === 0, failures: failures.map((f) => ({ path: templateEndpointPath(f.path), status: f.status, error: f.error })) },
    mode: 'live',
    generated_at: generatedAt,
    workspaces,
...
export async function getRouteScopedHonchoSnapshot(route = 'summary') {
  return createRouteScopedSnapshot(await getHonchoSnapshot(), route);
}
export { fetchJson };
