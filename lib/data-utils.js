...
export const LIVE_MESSAGE_BODY_REDACTION = '[redacted: live message body hidden without operator authentication]';
export const LIVE_MEMORY_TEXT_REDACTION = '[redacted: live memory text hidden without operator authentication]';
...
function safeWorkspace(workspace = {}) {
  ...
  return {
    ...
    summary: 'Live workspace details are hidden without operator authentication.'
  };
}
...
function safeSession(session = {}) {
  return {
    ...
    title: 'Live session details hidden without operator authentication',
...
function safeConclusion(conclusion = {}) {
  return {
    ...
    confidence_reason: conclusion.confidence_reason || 'Live memory text hidden without operator authentication',
...
export function createRouteScopedSnapshot(snapshot = {}, route = 'summary') {
  ...
  if (routeKey === 'home' || routeKey === 'dashboard') {
    ...
    scoped.workspaces = workspaces.slice(0, 20);
    scoped.peers = peers.slice(0, 50);
    scoped.messages = messages.slice(0, 10);
    scoped.conclusions = conclusions.slice(0, 20);
    scoped.kanban = compactKanban(snapshot.kanban, { includeAgents: true });
