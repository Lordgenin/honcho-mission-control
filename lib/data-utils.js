...
const PRIVATE_TEXT_PATTERNS = [
  /\b(?:[A-Z][A-Z0-9_]*(?:URL|URI|DSN|KEY|TOKEN|SECRET|PASSWORD|DATABASE)[A-Z0-9_]*|HONCHO_[A-Z0-9_]+|HERMES_[A-Z0-9_]+)\s*[:=]\s*[^\s,;)}\]]+/gi,
  /\b(?:postgres(?:ql)?|mysql|mariadb|redis|mongodb|sqlite):\/\/[^\s<>'"]+/gi,
  /\b(?:api[_-]?key|token|secret|password)\s+[A-Za-z0-9._~+\/-]{8,}/gi,
  /\bBearer\s+[A-Za-z0-9._~+\/-]{20,}/gi,
  /\b(?:sk|pk|ghp|gho|github_pat|hf)[_-][A-Za-z0-9_.\-]{8,}/gi,
  /https?:\/\/(?:localhost|host\.docker\.internal|127\.0\.0\.1|0\.0\.0\.0|10(?:\.\d{1,3}){3}|192\.168(?:\.\d{1,3}){2}|172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?::\d+)?(?:\/[^\s<>'"]*)?/gi,
  /\b(?:localhost|host\.docker\.internal|127\.0\.0\.1|0\.0\.0\.0|10(?:\.\d{1,3}){3}|192\.168(?:\.\d{1,3}){2}|172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?::\d+)?\b/gi,
  /\/(?:root|home|Users|etc|var|usr|opt)(?:\/[A-Za-z0-9._-]+){1,}(?:\.[A-Za-z0-9._-]+)?/g,
  /\b(?:private operator|operator note|local network|Proxmox|LXC|CT\d{2,4}|root@[A-Za-z0-9._-]+|internal port|migration status|provider key|auth mode)\b/gi,
  /\b(?:HONCHO_BASE_URL|HONCHO_API_KEY|DATABASE_URL|JWT|raw context JSON)\b/gi
];
...
function protectResourceCollectionPayload(payload, mapper, bodyKeys = ['content', 'text']) {
  const bodyKeySet = new Set(bodyKeys.map((key) => String(key).toLowerCase()));
  const hasBodyField = (value) => Object.entries(value).some(([key, item]) => {
    const normalizedKey = String(key).toLowerCase();
    if (!bodyKeySet.has(normalizedKey)) return false;
    return normalizedKey === 'content' || normalizedKey === 'text' || item === null || typeof item !== 'object';
  });
  const protectValue = (value) => {
    if (Array.isArray(value)) return value.map((item) => protectValue(item));
    if (!value || typeof value !== 'object') return sanitizePublicValue(value);
    if (hasBodyField(value)) return sanitizePublicValue(mapper(value));
    return sanitizePublicValue(Object.fromEntries(Object.entries(value)
      .filter(([key]) => String(key).toLowerCase() !== 'raw')
      .map(([key, item]) => [key, protectValue(item)])));
  };
  return protectValue(payload);
}
...
export function protectPublicProxyResponse(payload, path = [], viewerContext = {}) {
  ...
  if (isProxyResourcePath(path, 'messages')) return protectResourceCollectionPayload(payload, safeMessage, ['content', 'text', 'message', 'summary']);
  if (isProxyResourcePath(path, 'conclusions')) return protectResourceCollectionPayload(payload, safeConclusion, ['content', 'text', 'summary']);
...
export function getSubsystemStatuses(snapshot = {}) {
  ...
  const mode = getDashboardMode(snapshot);
  ...
  return [
    ...
    { id: 'public-privacy', label: 'Public privacy', state: mode.label, detail: mode.phrase }
  ];
}

export function getDashboardMode(snapshot = {}) {
  const source = String(snapshot.source || snapshot.mode || 'loading');
  const liveDataAllowed = snapshot.env?.liveDataAllowed === true;
  const readOnly = snapshot.readOnly !== false;
  const isLive = source.startsWith('live');
  const isPartial = source.includes('partial') || (isLive && snapshot.status?.ok === false);
  const demoRequested = !liveDataAllowed && (source === 'demo') && String(snapshot.env?.demoData || '').toLowerCase().includes('demo');

  if (liveDataAllowed && !readOnly && isLive && !isPartial) {
    return {
      label: 'operator-mutation-enabled',
      phrase: 'OPERATOR MUTATION ENABLED — write actions active',
      tone: 'border-rose-400/30 bg-rose-500/10 text-rose-200',
      summary: 'Trusted private deployment has explicitly enabled reviewed write-capable routes. Browser-visible secrets and raw diagnostics remain forbidden.',
      nextAction: 'Use confirmation, validation, and audit-safe feedback for every write action.',
      actionPosture: 'Write actions active only for reviewed private operator routes'
    };
  }
  if (liveDataAllowed && isPartial) {
    return {
      label: 'operator-live-partial',
      phrase: 'OPERATOR LIVE-PARTIAL — some sources unavailable',
      tone: 'border-amber-400/30 bg-amber-500/10 text-amber-200',
      summary: 'Live-private mode is enabled, but one or more upstream sources are unavailable or degraded; only sanitized failure reasons are shown.',
      nextAction: 'Inspect sanitized subsystem health and verify the private upstream from the protected deployment boundary.',
      actionPosture: 'Read-only live-private controls; mutations disabled'
    };
  }
  if (liveDataAllowed && isLive) {
    return {
      label: 'operator-live-private',
      phrase: 'OPERATOR LIVE-PRIVATE — scoped live data, read-only',
      tone: 'border-emerald-400/30 bg-emerald-500/10 text-emerald-200',
      summary: 'Scoped live data is visible for a trusted self-hosted deployment behind an external access boundary; mutations remain disabled.',
      nextAction: 'Review Agents and subsystem badges to confirm the expected scoped sources are visible.',
      actionPosture: 'Read-only live-private controls; mutations disabled'
    };
  }
  if (demoRequested) {
    return {
      label: 'public-demo',
      phrase: 'SANITIZED DEMO DATA — no live private instance connected',
      tone: 'border-sky-400/30 bg-sky-500/10 text-sky-200',
      summary: 'The dashboard is using bundled synthetic sample data; it is safe to explore and is not connected to private Honcho memory.',
      nextAction: 'Use the demo to evaluate the product, then self-host behind an access boundary before enabling scoped live data.',
      actionPosture: 'Demo-only controls; writes disabled'
    };
  }
  if (!liveDataAllowed) {
    return {
      label: 'public-protected',
      phrase: 'PUBLIC PRIVACY PROTECTED — live memory hidden',
      tone: 'border-sky-400/30 bg-sky-500/10 text-sky-200',
      summary: 'Server-side services may be configured, but live private memory is fail-closed and browser-visible data is sanitized.',
      nextAction: 'Keep public deployments read-only and use placeholder-safe settings until moved behind a private operator boundary.',
      actionPosture: 'Protected read-only preview; live memory and writes disabled'
    };
  }
  return {
    label: 'loading',
    phrase: 'LOADING — dashboard posture pending',
    ...
    actionPosture: 'Actions unavailable until posture resolves'
  };
}
...
export function getSnapshotPosture(snapshot = {}) {
  ...
  return {
    ...
    phrase: mode.phrase,
    actionPosture: mode.actionPosture
  };
}
