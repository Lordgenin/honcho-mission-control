...
export function getHealthPayload({ now = new Date(), envSource = process.env } = {}) {
  ...
  return {
    ...
    kanban: {
      ...
      container_mount: CONTAINER_KANBAN_DB,
      container_mount_readable: canReadContainerKanbanDb(),
      source_label: kanbanConfigured ? 'container-mounted-db' : 'not-configured'
    }
  };
}
