...
function isStaticKanbanSnapshot(envSource = {}) {
  return Boolean(envSource.HERMES_KANBAN_SNAPSHOT_HOST_DB || envSource.HERMES_KANBAN_SNAPSHOT_MODE === 'static');
}
...
export function getHealthPayload({ now = new Date(), envSource = process.env, exposeOperatorDiagnostics = false } = {}) {
  ...
  const payload = {
    ...
    kanban: {
      ...
      source_mode: staticKanbanSnapshot ? 'static-snapshot' : (kanbanConfigured ? 'mounted-db' : 'not-configured'),
