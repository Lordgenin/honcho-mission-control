...
function sourceStatus({ index = 0, reason = 'configured', ok = false, rowCount = 0, rows = null, error = '' } = {}) {
...
path = sys.argv[1]
con = sqlite3.connect('file:' + path + '?mode=ro&immutable=1', uri=True)
...
export function getKanbanRuntimeSnapshot({ env = process.env, generatedAt = new Date().toISOString(), execFileSyncImpl = execFileSync } = {}) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      sources.push(sourceStatus({ index: candidate.index, reason: candidate.reason, ok: true, rowCount: Array.isArray(sourceRows.tasks) ? sourceRows.tasks.length : 0, rows: sourceRows }));
    } catch (error) {
      lastError = error;
