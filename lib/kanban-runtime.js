...
export function summarizeKanbanRuntimeRows({ tasks = [], runs = [], events = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  return {
    available: true,
    state: agents.length ? 'available' : 'unknown',
...
export function buildDegradedKanbanRuntime({ generatedAt = new Date().toISOString(), reason = '' } = {}) {
...
function defaultDbPath(env = process.env) {
  return env.HERMES_KANBAN_DB || env.HERMES_KANBAN_DATABASE || path.join(os.homedir(), '.hermes', 'kanban.db');
}
...
export function readKanbanRows({ dbPath = defaultDbPath(), execFileSyncImpl = execFileSync } = {}) {
...
export function getKanbanRuntimeSnapshot({ env = process.env, generatedAt = new Date().toISOString(), execFileSyncImpl = execFileSync } = {}) {
  try {
    const rows = readKanbanRows({ dbPath: defaultDbPath(env), execFileSyncImpl });
    return summarizeKanbanRuntimeRows({ ...rows, generatedAt });
  } catch (error) {
    return buildDegradedKanbanRuntime({ generatedAt, reason: error?.message || 'Kanban runtime unavailable' });
  }
}
