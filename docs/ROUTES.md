...
- `/dashboard` - System status summary: source, API health, generated timestamp, read-only/mutation posture, workspace scope, agent counts, and recent memory activity.
...
- `/performance` - Traceable performance metrics when available; empty state is shown when live telemetry is not configured.
