...
export function AgentsView({ snapshot }: { snapshot: any }) {
  ...
  return <>
    ...
    {peerFailure ? <PeerDiscoveryFailure failure={peerFailure}/> : agents.length ? <>
      <div className="mb-4 grid gap-4 md:grid-cols-3"><Stat label="Discovered agents" value={agents.length} helper="Kanban runtime plus Honcho peers"/><Stat label="Kanban state" value={snapshot.kanban?.state || 'not attached'} helper={snapshot.kanban?.source || 'source not reported'}/><Stat label="Activity reported" value={agents.filter((a:any)=>a.last_seen || a.heartbeat).length} helper="Safe Kanban/Honcho timestamps only"/></div>
